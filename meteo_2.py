...
for semana in range(semanas):
	n_input_steps = 24 * 7 * semanas[semana]
	n_output_steps = 24 * 3
	# pre-process data
	...
	with open("h_input_"+str(semana)+"_hidden_"+str(n_LSTM_hidden_layers)+"_cells_"+str(n_cells)+ mls_label[param], mode='w') as f:
		  hist_df.to_json(f)
	mls[param].model.save("m_" + mls_label[param] + ".h5")
	...
	print('Test Score ' + mls_label[param] + ': %.2f RMSE' % (testScore))
